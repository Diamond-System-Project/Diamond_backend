...
public class CollectionController {
    ...
    public ResponseEntity<ApiResponse> createCollection(@Valid @RequestBody CollectionDTO collectionDTO) {
        try {
            ...
            return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR).body(
                    ApiResponse.builder()
                            .success(false)
                            .message("Error: Could not create the collection")
                            .build());
        }
    }
    ...
    public ResponseEntity<ApiResponse> updateCollection(@PathVariable Integer collectionId, @Valid @RequestBody CollectionDTO collectionDTO) {
        try {
            ...
            return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR).body(
                    ApiResponse.builder()
                            .success(false)
                            .message("Error: Could not update the collection")
                            .build());
        }
    }
    ...
    @DeleteMapping("delete/{collectionId}/product/{productId}")
    @PreAuthorize("hasRole('ROLE_Manager')")
    public ResponseEntity<ApiResponse> removeProductFromCollection(@PathVariable Integer collectionId, @PathVariable Integer productId) {
        try {
            if (!collectionService.existCollection(collectionId)) {
                return ResponseEntity.status(HttpStatus.NOT_FOUND).body(
                        ApiResponse.builder()
                                .success(false)
            ...
            if (!collectionService.existProduct(productId)) {
                return ResponseEntity.status(HttpStatus.NOT_FOUND).body(
                        ApiResponse.builder()
                                .success(false)
